...
get_cluster_robust_vcov  = function(V, u, cluster) {

  clusters = unique(cluster)
  G = length(clusters)

  meat = matrix(0, nrow = ncol(V), ncol = ncol(V))  # r x r

  for (g in clusters) {
    g_idx = which(cluster == g)

    Vg = V[g_idx, , drop = FALSE]    # T x r
    ug = u[g_idx, drop = FALSE]      # T x 1

    Sg = t(Vg) %*% ug                # r x 1
    meat = meat + Sg %*% t(Sg)       # add outer product
  }

  meat = meat * G / (G - 1)

  QVVinv = solve(t(V) %*% V)   #rxr
  vcov   = QVVinv %*% meat %*% QVVinv   # r x r
  dimnames(vcov) = list(colnames(V), colnames(V))

  return(vcov)
}

arcset = function(res_y, res_d, res_z,
                  res_delta, res_pi, r, adj) {

  # Calculate AR confidence sets (based on twostepweakiv/weakiv commands in Stata)
  #___________________________________________________________#
  # type       Case               Confidence set
  #___________________________________________________________#
  # 1			  D<0 & a22>0             Empty set
  # 2			  D>0 & a22<0            [ x1, x2 ]
  # 3			  D<0 & a22<0            (-infty, +infty)
  # 4		    D>0 & a22>0            (-infty, x1] U [x2, infty)
  #___________________________________________________________#

  # Get the average of the residuals from
  #    (a) res_delta: reg Y-l on Z-m => Uhat - delta'Vhat (Nx1)
  #    (b) res_pi:    reg D-r on Z-m => Rhat - pi'Vhat    (Nx1)

  R = cbind(res_z,res_y,res_d)
  cross=crossprod(R)
  zpz = cross[1:r, 1:r]
  zpy = cross[1:r, (r+1):(r+2), drop = FALSE]
  ypz = t(zpy)
  M     = cbind(res_delta, res_pi)        # N x 2 residual matrix
  omega = crossprod(M) / adj              # 2 x 2 matrix: t(M) %*% M
  kcv1 = qchisq(0.95, df = r)

  # if (r==1){
  #   zpz = matrix(zpz, nrow = 1, ncol = 1)
  #   zpy = matrix(zpy, nrow = 1, ncol = 2)
  #   ypz = matrix(ypz, nrow = 2, ncol = 1)
  # }
  A = ypz %*% solve(zpz) %*% zpy - kcv1*omega #works with r>1
  ...
  a22 = A[2,2]
   ...
   if (a22 < 0){
     if (D < 0){
       ...
       xx1 = -Inf
       xx2 = Inf
       txt_ar_cset = paste0("( -inf, +inf )")
     }else{
       type=4
       xx1 = round((A[1,2] + sqrt(D))/a22, digits = 3)
       xx2 = round((A[1,2] - sqrt(D))/a22, digits = 3)
       txt_ar_cset = paste0("( -inf, ",  xx1 , " ] U [ ", xx2, " , +inf )")
     }
   }else{
     if (D < 0){
       type = 1
       xx1 = NA_real_
       xx2 = NA_real_
       txt_ar_cset = paste0("empty set")
     }else{
       type = 2
       xx1 = round((A[1,2]-sqrt(D))/a22, digits=3)
       xx2 = round((A[1,2]+sqrt(D))/a22, digits=3)
       txt_ar_cset = paste0("[ ", xx1 , " , ", xx2, " ]")
     }
   }
#  print(paste0("txt_ar_cset: ", txt_ar_cset))
  ar_cset = list(xx1 = xx1, xx2 = xx2, txt_ar_cset = txt_ar_cset, type = type)
  return(ar_cset)
}
