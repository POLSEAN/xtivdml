...
#'
#' @format [R6::R6Class] object inheriting from [xtivdml].
#'
#' @family xtivdml
#'
#' @details
...
#'
#' @export
xtivdml_plr = R6Class("xtivdml_plr",
  inherit = xtivdml,
  active = list(
  ...
  public = list(
    ...
    #'
    #' @param data (`xtivdml_data`) \cr
    #' The `xtivdml_data` object providing the data and specifying the variables
    #' of the causal model.
    #'
    ...
    #' learning algorithm. Default is `"dml1"`.
    #'
    ...
    initialize = function(data,
      ...
      dml_procedure = "dml1",   #dml2 not allowed with pliv
    ...
    #' @description
    #' Set hyperparameters for the nuisance models of xtivdml models.
    #'
    ...
    #' @description
    #' Hyperparameter-tuning for xtivdml models.
    #'
  ...
  private = list(
    ...
    nuisance_est_partialX = function(smpls, ...) {
      ...
      if (self$data$n_instr == 1) {
        ...
        models[['ml_m']]  = m_hat$models
         ...
         psi_pi_a = array(NA_real_, dim = c(self$data$n_instr))
         psi_pi_b = array(NA_real_, dim = c(self$data$n_instr))
         pi_hat = array(NA_real_, dim = c(self$data$n_instr))
    ...
    panel_score_elements = function(y, z, d,
      ...
      if (self$data$n_instr == 1) {
        if (self$score == "partialling out") {
          # For 2nd stage (Y on D)
          psi_theta_a = -v_hat * w_hat
          psi_theta_b =  v_hat * u_hat
          # For 1st stage (D on Z)
          psi_pi_a    = -v_hat * v_hat
          psi_pi_b    =  v_hat * w_hat
          # For reduced form (Y on Z)
          psi_delta_a = -v_hat * v_hat    # Added Feb 2025
          psi_delta_b =  v_hat * u_hat    # Added Feb 2025
          # Residuals
          res_y = u_hat
          res_d = w_hat
          res_z = v_hat
          ...
          print("IV-type not allowed with PLIV")
          # psi_theta_a = -d * v_hat
          # psi_theta_b =  v_hat * (y - g_hat)
          # psi_pi_a = -v_hat * z
          # psi_pi_b =  v_hat * w_hat # w_hat = d - rr_hat
          # res_y = y - g_hat
          # res_d = d
          # res_z = v_hat
        }

        # Second stage coefficient of the treatment (causal parameter)
        theta_hat = -mean(psi_theta_b, na.rm = TRUE) / mean(psi_theta_a, na.rm = TRUE)

        # First stage coefficient of the instrument
        pi_hat = -mean(psi_pi_b, na.rm = TRUE) / mean(psi_pi_a, na.rm = TRUE)

        # Reduced-form coefficient of the instrument
        delta_hat = -mean(psi_delta_b, na.rm = TRUE) / mean(psi_delta_a, na.rm = TRUE) # Added Feb 2025
        ...
        res = list(theta_hat = theta_hat,
                   pi_hat    = pi_hat,
                   delta_hat = delta_hat,    #added Feb2025
                   model_mse = model_mse,
                   res_y = res_y,
                   res_d = res_d,
                   res_z = res_z)
        #print(paste0("pi in approx_pliv: ", res$pi_hat))
        psis = list(psi_theta_a = psi_theta_a,
                    psi_theta_b = psi_theta_b,
                    psi_pi_a    = psi_pi_a,
                    psi_pi_b    = psi_pi_b,
                    psi_delta_a = psi_delta_a, # Added Feb 2025
                    psi_delta_b = psi_delta_b)
        return(c(res,psis))
         ...
         pi_hat = solve(t(V) %*% V) %*% (t(V) %*% W)
         delta_hat = solve(t(V) %*% V) %*% (t(V) %*% U)
        ...
        if (self$score == "partialling out") {
          ...
          res_y = u_hat
          res_d = w_hat
          res_z = v_hat #r_hat_tilde
          theta_hat = -mean(psi_theta_b, na.rm = TRUE) / mean(psi_theta_a, na.rm = TRUE)
          pi_hat    = pi_hat
          delta_hat = delta_hat  #-mean(psi_delta_b, na.rm = TRUE) / mean(psi_delta_a, na.rm = TRUE)  #feb2025
        ...
        psis = list(psi_theta_a = psi_theta_a,
                    ...
                    psi_pi_a = psi_pi_a,
                    psi_pi_b = psi_pi_b,
                    psi_delta_a = psi_delta_a,
                    psi_delta_b = psi_delta_b)
        return(c(res,psis))
      }
    },
    nuisance_est_partialXZ = function(smpls, ...) {  #not updated
    ...
    nuisance_est_partialZ = function(smpls, ...) {
      ...
      if (is.character(self$score)) {
        ...
        stop(paste(
          "Callable score not implemented for xtivdml_plr",
    ...
    nuisance_tuning_partialX = function(smpls, param_set,
      ...
      if (self$data$n_instr == 1) {
        ...
        if (exists("ml_g", where = private$learner_)) { #remove
          if (tune_on_folds) {
        ...
        tuning_result[["ml_l"]] = list(tuning_result_l, params = tuning_result_l$params)
    ...
    check_data = function(obj_dml_data) {
      if (obj_dml_data$n_instr == 0) {
        stop(paste(
          ...
          "variable(s) use xtivdmlPLR instead of xtivdml_plr."))
      }
      return()
    }
  )
  ...
  xtivdml_plr.partialX = function(data,
  ml_l,
  ml_m,
  ml_r,
  ml_g = NULL,
  ...
  xtivdml_plr.partialZ = function(data,
  ml_r,
  n_folds = 5,
  n_rep = 1,
  score = "partialling out",
  ...
  xtivdml_plr.partialXZ = function(data,
  ml_l,
  ml_m,
  ml_r,
  n_folds = 5,
